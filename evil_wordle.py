...
On my/our honor, Neil Sebastian , this
...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        """
        for letter, color in zip(guessed_word, feedback_colors):
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    unmatched_secret_letters = defaultdict(int)

    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            feedback[i] = CORRECT_COLOR
        else:
            unmatched_secret_letters[secret_word[i]] += 1

    for i in range(NUM_LETTERS):
        if feedback[i] == NOT_IN_WORD_COLOR and unmatched_secret_letters[guessed_word[i]] > 0:
            feedback[i] = WRONG_SPOT_COLOR
            unmatched_secret_letters[guessed_word[i]] -= 1
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        ...
        word_families[feedback].append(word)

    word_family_objects = [WordFamily(pattern, words) for pattern, words in word_families.items()]
    sorted_families = fast_sort(word_family_objects)
